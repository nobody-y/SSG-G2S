...
class SGRecallInfo(SceneGraphEvaluation):
    def __init__(self, result_dict, num_rel_category, ind_to_predicates):
        super(SGRecallInfo, self).__init__(result_dict)
...
class SGAmountInfo(SceneGraphEvaluation):
    def __init__(self, result_dict, num_rel_category, ind_to_predicates):
        super(SGAmountInfo, self).__init__(result_dict)
        self.num_rel_category = num_rel_category
        self.ind_to_predicates = ind_to_predicates
        vg_dict_info = json.load(open('./datasets/vg/VG-SGG-dicts-with-attri-info.json','r'))
        predicates_info = vg_dict_info['predicate_information']
        wiki_info = json.load(open('./datasets/vg/WIKIPEDIA-info.json','r'))
        predicates_wiki_info = wiki_info['predicate_wiki_information']
        pred_info_arr = []
        for i in range(len(self.ind_to_predicates)):
            pred_i = self.ind_to_predicates[i]
            if pred_i in predicates_info:
    ...
    def register_container(self, mode):
        self.result_dict['predicate_amount_information'] = None
        self.result_dict['predicate_amount_information_wiki'] = None

    def generate_print_string(self, mode):
        result_str = 'SGG amount of VG information: %.4f ' % self.result_dict['predicate_amount_information']
        result_str += '\n'
        result_str = 'SGG amount of WIKIPEDIA information: %.4f ' % self.result_dict['predicate_amount_information_wiki']
        result_str += '\n'
        return result_str

    def calculate_amount_information(self, global_container, local_container, mode):
        rel_scores = local_container['rel_scores']
        rel_info = rel_scores * self.pred_info_arr[None, :]
        rel_info = rel_info.mean()
        self.result_dict['predicate_amount_information'] = rel_info

        rel_wiki_info = rel_scores * self.pred_wiki_info_arr[None, :]
        rel_wiki_info = rel_wiki_info.mean()
        self.result_dict['predicate_amount_information_wiki'] = rel_wiki_info
