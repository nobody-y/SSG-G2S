...
from maskrcnn_benchmark.data.datasets.evaluation.vg.sgg_eval import SGAmountInfo, SGRecallInfo
def do_vg_evaluation(
    ...
    if "relations" in iou_types:
        ...
        eval_recall_info = SGRecallInfo(result_dict, num_rel_category, dataset.ind_to_predicates)
        eval_recall_info.register_container(mode)
        evaluator['eval_recall_information'] = eval_recall_info
        ...
        eval_amount_info = SGAmountInfo(result_dict, num_rel_category, dataset.ind_to_predicates)
        eval_amount_info.register_container(mode)
        evaluator['eval_amount_information'] = eval_amount_info
        ...
        result_str += eval_amount_info.generate_print_string(mode)
        if cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    local_container = evaluator['eval_recall_information'].calculate_recallinfo(global_container, local_container, mode)
    # No Graph Constraint
    ...
    evaluator['eval_amount_information'].calculate_amount_information(global_container, local_container, mode)
    # Mean Recall
